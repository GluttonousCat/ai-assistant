...
用法(命令行):
    python -m beta_alpha.forge "苹果代工产业链"            # 生成并保存
    python -m beta_alpha.forge "磷化工" --no-save          # 只看草稿
    python -m beta_alpha.forge --check ai_compute          # 校验已有链
"""
...
from beta_alpha.prompts import CHAIN_FORGE_PROMPT, CHAIN_REFINE_PROMPT
...
def chain_to_yaml(chain: Dict) -> str:
    ...
    return (f"# source: forge 保存于 {date.today().isoformat()}\n"
            f"# 命中率复查: python -m beta_alpha.forge --check {chain.get('chain_id')}\n"
            + body)
...
def main() -> None:
    ...
    if not args.no_save:
        ...
        print(f"预览: python -m beta_alpha.analysis.chain_analysis {res['chain']['chain_id']}")
