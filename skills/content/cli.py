...
def main() -> int:
    ...
    args = p.parse_args()
    if args.cmd == "profile":
        from skills.content.profile import build_company_profile, profile_digest
    ...
    if args.cmd == "interpret":
        from skills.content.article import write_stock_article
        ...
        print("\n---- 预览 ----\n", res["preview"])
        if args.ppt:
            from skills.content.ppt import generate_profile_ppt
        ...
        return 0
    if args.cmd == "ppt":
        from skills.content.ppt import generate_profile_ppt, make_task_book
